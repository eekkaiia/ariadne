...
async fn main() {
    ...
    let mut portalia = Portalia::new();
    portalia.fill_maze(&maze);
    eprintln!("Starting game loop...");
    loop { // macroquad game loop
        control.update(&mut stage, &mut maze, &mut theseus, &mut paraphernalia, &mut portalia);
        portalia.update();
        paraphernalia.update(&mut theseus, &maze);
        stage.update(&mut info, &mut control, &mut maze, &mut theseus, &paraphernalia, &portalia);
